# filepathutil

A package filepathutil implements a `path/filepath` utility.

|                   | **CI Service**                                |
|-------------------|-----------------------------------------------|
| **linux**         | [![travis-ci.org][travis-badge]][travis-ci]   |
| **darwin**        | [![circleci.com][circleci-badge]][circleci]   |
| **windows**       | [![appveyor-badge][appveyor-badge]][appveyor] |

[![codecov.io][codecov-badge]][codecov] [![godoc.org][godoc-badge]][godoc] [![Analytics][ga-badge]][ga]


## Install

```sh
go get -u github.com/pkgutil/filepathutil
```

## Overview

```go
import "github.com/pkgutil/filepathutil"
```

## <a name="pkg-index">Index</a>
* [func IsDir(filename string) bool](#IsDir)
* [func IsExist(filename string) bool](#IsExist)
* [func IsNotExist(filename string) bool](#IsNotExist)
* [func MkdirAll(dir string, perm os.FileMode) error](#MkdirAll)

### <a name="pkg-files">Package files</a>
[filepathutil.go](/src/github.com/pkgutil/filepathutil/filepathutil.go) 


## <a name="IsDir">func</a> [IsDir](/src/target/filepathutil.go?s=590:622#L14)
``` go
func IsDir(filename string) bool
```
IsDir returns whether the filename is directory.



## <a name="IsExist">func</a> [IsExist](/src/target/filepathutil.go?s=256:290#L2)
``` go
func IsExist(filename string) bool
```
IsExist returns whether the filename is exists.



## <a name="IsNotExist">func</a> [IsNotExist](/src/target/filepathutil.go?s=425:462#L8)
``` go
func IsNotExist(filename string) bool
```
IsNotExist returns whether the filename is not exists.



## <a name="MkdirAll">func</a> [MkdirAll](/src/target/filepathutil.go?s=794:843#L20)
``` go
func MkdirAll(dir string, perm os.FileMode) error
```
MkdirAll checks whether the exist dir directory. and create directory to dir filepath if not exist.


[travis-ci]: https://travis-ci.org/pkgutil/filepathutil
[circleci]: https://circleci.com/gh/pkgutil/filepathutil
[appveyor]: https://ci.appveyor.com/project/zchee/filepathutil
[godoc]: https://godoc.org/github.com/pkgutil/filepathutil
[codecov]: https://codecov.io/gh/pkgutil/filepathutil
[ga]: https://github.com/pkgutil/filepathutil

[travis-badge]: https://img.shields.io/travis/pkgutil/filepathutil.svg?style=flat-square&label=%20Travis%20CI&logo=data%3Aimage%2Fsvg%2Bxml%3Bcharset%3Dutf-8%3Bbase64%
[circleci-badge]: https://img.shields.io/circleci/project/github/pkgutil/filepathutil.svg?style=flat-square&label=%20%20CircleCI&logoWidth=16&logo=data%3Aimage%2Fsvg%2Bxml%3Bcharset%3Dutf-8%3Bbase64%%3D%3D
[appveyor-badge]: https://img.shields.io/appveyor/ci/pkgutil/filepathutil/master.svg?style=flat-square&label=AppVeyor&logo=data%3Aimage%2Fsvg%2Bxml%3Bcharset%3Dutf-8%3Bbase64%%3D%3D
[godoc-badge]: https://img.shields.io/badge/godoc-reference-4F73B3.svg?style=flat-square&label=%20godoc.org
[codecov-badge]: https://img.shields.io/codecov/c/github/pkgutil/filepathutil.svg?style=flat-square&label=%20Codecov%2Eio&logo=data%3Aimage%2Fsvg%2Bxml%3Bcharset%3Dutf-8%3Bbase64%
[ga-badge]: https://ga-beacon.appspot.com/UA-89201129-1/pkgutil/filepathutil?flat&useReferer&pixel
